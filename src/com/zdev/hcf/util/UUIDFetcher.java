...
public class UUIDFetcher implements Callable<Map<String, UUID>> {
	...
	public Map<String, UUID> call() throws Exception {
		...
		for (int requests = (int) Math.ceil(this.names.size() / 100.0), i = 0; i < requests; ++i) {
			...
			final String body = JSONArray.toJSONString(
					(List<String>) this.names.subList(i * 100, Math.min((i + 1) * 100, this.names.size())));
			writeBody(connection, body);
			final JSONArray array = (JSONArray) this.jsonParser
					.parse((Reader) new InputStreamReader(connection.getInputStream(), StandardCharsets.UTF_8));

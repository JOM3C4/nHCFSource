...
public class DoubleKeyCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
			...
			sender.sendMessage("§b/dkey start <duration> - Starts the DoubleKey timer.");
			sender.sendMessage("§b/dkey end - Ends the current DoubleKey timer.");
			sender.sendMessage("§7§m--------------------------------");
			return true;
		}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				Bukkit.broadcastMessage(ChatColor.GRAY + "The store-wide " + ChatColor.DARK_RED.toString()
						+ ChatColor.BOLD + "DoubleKey " + ChatColor.GRAY + "has started at " + ChatColor.DARK_AQUA

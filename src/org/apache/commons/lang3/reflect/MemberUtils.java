...
/**
 * Contains common code for working with {@link java.lang.reflect.Method
 * Methods}/{@link java.lang.reflect.Constructor Constructors}, extracted and
...
abstract class MemberUtils {
	 ...
	 * compiled code works fine. Unfortunately, on some JVMs, using reflection to
	 * invoke these members seems to (wrongly) prevent access even when the modifier
	 * is {@code public}. Calling {@code setAccessible(true)} solves the problem but
	 * will only work from sufficiently privileged code. Better workarounds would be
	 * gratefully accepted.
	 * 
	 * @param o the AccessibleObject to set as accessible
	 * @return a boolean indicating whether the accessibility of the object was set
	 *         to true.
	 */
	 ...
	 * @return {@code true} unless {@code package}/{@code protected}/{@code private}
	 *         modifier detected
	 */
	...
	/**
	 * Compares the relative fitness of two Methods in terms of how well they match
	 * a set of runtime parameter types, such that a list ordered by the results of
	...
	private static float getTotalTransformationCost(final Class<?>[] srcArgs, final Executable executable) {
		...
		if (isVarArgs) {
			...
			if (noVarArgsPassed) {
				// When no varargs passed, the best match is the most generic matching type, not
				// the most specific.
	...
	/**
	 * <p>
	 * A class providing a subset of the API of java.lang.reflect.Executable in Java
	 * 1.8, providing a common representation for function signatures for

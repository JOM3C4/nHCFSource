 ...
 * default access superclass. Reflection correctly locates these constructors
 * and assigns them as {@code public}. However, an
 * {@link IllegalAccessException} is thrown if the constructor is invoked.
 * </p>
 *
 * <p>
 * {@link ConstructorUtils} contains a workaround for this situation: it will
 * attempt to call
 * {@link java.lang.reflect.AccessibleObject#setAccessible(boolean)} on this
 * constructor. If this call succeeds, then the method can be invoked as normal.
 * This call will only succeed when the application has sufficient security
 * privileges. If this call fails then a warning will be logged and the method
 * may fail.
 * </p>
 *
 * @since 2.5
...
public class ConstructorUtils {
	 ...
	 * @param cls            the class to be constructed, not {@code null}
	 * @param args           the array of arguments, {@code null} treated as empty
	 * @param parameterTypes the array of parameter types, {@code null} treated as
	 *                       empty
	 * @return new instance of {@code cls}, not {@code null}
	 *
	 * @throws NullPointerException      if {@code cls} is {@code null}
	 ...
	 * @param parameterTypes the array of parameter types, {@code null} treated as
	 *                       empty
	 * @return the constructor, {@code null} if no matching accessible constructor
	 *         found
	...
	public static <T> Constructor<T> getMatchingAccessibleConstructor(final Class<T> cls,
		...
		/*
		 * (1) Class.getConstructors() is documented to return Constructor<T> so as long
		 * as the array is not subsequently modified, everything's fine.
		 */
	...
	/**
	 * Learn whether the specified class is generally accessible, i.e. is declared
	 * in an entirely {@code public} manner.
	 * 
	 * @param type to check

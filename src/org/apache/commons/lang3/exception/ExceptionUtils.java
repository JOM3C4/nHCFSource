...
public class ExceptionUtils {
	 ...
	 *
	 * <p>
	 * This may be modified and used in the overloaded getCause(Throwable, String[])
	 * method.
	 * </p>
	 *
	 ...
	 * <p>
	 * From version 2.2, this method handles recursive cause structures that might
	 * otherwise cause infinite loops. The cause chain is processed until the end is
	 ...
	 * input throwable. A throwable with one cause will return a list containing two
	 * elements. - the input throwable and the cause throwable. A <code>null</code>
	 ...
	 * the specified class do not match - see {@link #indexOfType(Throwable, Class)}
	 * for the opposite.
	 * </p>
	 *
	 * <p>
	 ...
	 * the specified class do match - see
	 ...
	 *                  using references
	 * @return index of the <code>type</code> within throwables nested within the
	 ...
	 * Only the trace of the specified exception is returned, any caused by trace is
	 * stripped.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * Throw a checked exception without adding the exception to the throws clause
	 * of the calling method. This method prevents throws clause pollution and
	 * reduces the clutter of "Caused by" exceptions in the stacktrace.
	 * <p>
	 ...
	 * </code>
	 * <p>
	 * One downside to using this approach is that the java compiler will not allow
	 * invoking code to specify a checked exception in a catch clause unless there
	 * is some code path within the try block that has invoked a method declared
	 * with that checked exception. If the invoking site wishes to catch the shaded
	 * checked exception, it must either invoke the shaded code through a method
	 * re-declaring the desired checked exception, or catch Exception and use the
	 * instanceof operator. Either of these techniques are required when interacting
	 * with non-java jvm code such as Jython, Scala, or Groovy, since these
	 * languages do not consider any exceptions as checked.
	 *
	 * @param throwable The throwable to rethrow.
	 * @param <R>       The type of the returned value.
	 * @return Never actually returned, this generic type matches any type which the
	 *         calling site requires. "Returning" the results of this method, as
	...
	/**
	 * Claim a Throwable is another Exception type using type erasure. This hides a
	 * checked exception from the java compiler, allowing a checked exception to be
	 * thrown without having the exception in the method's throw clause.
	 */
	...
	/**
	 * Throw a checked exception without adding the exception to the throws clause
	 * of the calling method. For checked exceptions, this method throws an
	 * UndeclaredThrowableException wrapping the checked exception. For Errors and
	 * RuntimeExceptions, the original exception is rethrown.
	 * <p>
	 ...
	 * @param <R>       The type of the returned value.
	 * @return Never actually returned, this generic type matches any type which the
	 *         calling site requires. "Returning" the results of this method will
	 *         satisfy the java compiler requirement that all code paths return a
	 *         value.
	 * @since 3.5
	...
	/**
	 * Does the throwable's causal chain have an immediate or wrapped exception of
	 * the given type?
	 *

...
public class TimedSemaphore {
	...
	/**
	 * Creates a new instance of {@link TimedSemaphore} and initializes it with the
	 * given time period and the limit.
	 *
	 ...
	 * executor service, the given time period, and the limit. The executor service
	 * will be used for creating a periodic task for monitoring the time period. It
	 * can be <b>null</b>, then a default service will be created.
	 *
	...
	/**
	 * Sets the limit. This is the number of times the {@link #acquire()} method can
	 * be called within the time period specified. If this limit is reached, further
	 * invocations of {@link #acquire()} will block. Setting the limit to a value
	 * &lt;= {@link #NO_LIMIT} will cause the limit to be disabled, i.e. an
	 * arbitrary number of{@link #acquire()} invocations is allowed in the time
	 * period.
	 *
	 ...
	 * method can be invoked an arbitrary number of times. All invocations after the
	...
	/**
	 * Acquires a permit from this semaphore. This method will block if the limit
	 * for the current period has already been reached. If {@link #shutdown()} has
	 * already been invoked, calling this method will cause an exception. The very
	 * first call of this method starts the timer task which monitors the time
	 * period set for this {@code TimedSemaphore}. From now on the semaphore is
	 * active.
	 *
	...
	/**
	 * Tries to acquire a permit from this semaphore. If the limit of this semaphore
	 * has not yet been reached, a permit is acquired, and this method returns
	 * <strong>true</strong>. Otherwise, this method returns immediately with the
	 ...
	 * period. This is the number of times the {@link #acquire()} method was called
	 * without blocking. This can be useful for testing or debugging purposes or to
	 * determine a meaningful threshold value. If a limit is set, the value returned
	 * by this method won't be greater than this limit.
	 *
	 ...
	 * indication whether it is safe to call the {@link #acquire()} method without
	 * risking to be suspended. However, there is no guarantee that a subsequent
	 * call to {@link #acquire()} actually is not-blocking because in the mean time
	 ...
	 * given number of invocations of the {@link #acquire()} method is possible in
	 * this period.
	 *
	...
	/**
	 * Starts the timer. This method is called when {@link #acquire()} is called for
	 * the first time. It schedules a task to be executed at fixed rate to monitor
	 ...
	 * internally to monitor the time period. It resets the counter and releases the
	 * threads waiting for this barrier.
	 */
	 ...
	 * internal timer if necessary. This method must be called with the lock of this
	 * object held.
	 */
	 ...
	 * counter. The return value indicates whether a permit could be acquired. This
	 * method must be called with the lock of this object held.
	 *

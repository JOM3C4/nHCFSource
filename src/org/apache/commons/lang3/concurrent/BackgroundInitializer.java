...
public abstract class BackgroundInitializer<T> implements ConcurrentInitializer<T> {
	 ...
	 * with the given {@code ExecutorService}. If the {@code ExecutorService} is not
	 * null, the background task for initializing this object will be scheduled at
	 * this service. Otherwise a new temporary {@code
	 ...
	 * ExecutorService} passed to this method is used for executing the background
	 * task. Thus it is possible to re-use an already existing
	 ...
	 * task. A {@code BackgroundInitializer} can be started exactly once. The return
	 * value of this method determines whether the start was successful: only the
	 * first invocation of this method returns <b>true</b>, following invocations
	...
	/**
	 * Returns the result of the background initialization. This method blocks until
	 * initialization is complete. If the background processing caused a runtime
	 * exception, it is directly thrown by this method. Checked exceptions,
	 * including {@code InterruptedException} are wrapped in a
	 * {@link ConcurrentException}. Calling this method before {@link #start()} was
	 * called causes an {@code IllegalStateException} exception to be thrown.
	 *
	...
	/**
	 * Returns the {@code Future} object that was created when {@link #start()} was
	 * called. Therefore this method can only be called after {@code
	 * start()}.
	 *
	 ...
	 * this is also the active executor. Otherwise this method returns the temporary
	 * executor that was created by this object.
	 *
	 ...
	 * This information is evaluated when a temporary {@code
	 ...
	 * method is called from a synchronized block by the {@link #start()} method.
	 * Therefore overriding methods should be careful with obtaining other locks and
	...
	/**
	 * Performs the initialization. This method is called in a background task when
	 * this {@code BackgroundInitializer} is started. It must be implemented by a
	 * concrete subclass. An implementation is free to perform arbitrary
	...
	/**
	 * Creates a task for the background initialization. The {@code Callable} object
	 * returned by this method is passed to the {@code ExecutorService}. This
	 * implementation returns a task that invokes the {@link #initialize()} method.
	 * If a temporary {@code ExecutorService} is used, it is destroyed at the end of
	 * the task.
	 *
	...
	private class InitializationTask implements Callable<T> {
		...
		/**
		 * Creates a new instance of {@code InitializationTask} and initializes it with
		 * the {@code ExecutorService} to be destroyed at the end.
		 *
